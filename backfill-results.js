 ...
 * backfill-results.js
 *
 * Same approach as backfill-standings.js, applied to `public.results`:
 * fills in missing seasons for the 122 merged domestic leagues in
 * SELECTED_LEAGUE_IDS (union of your moonbet_domestic_leagues.xlsx
 * shortlist and sync_fixtures_odds.js's live cron list), going back to
 * 2009, without re-fetching seasons you already have. Existing rows are
 * never touched.
 *
 * Row shape matches the current `results` table AFTER home_mg/away_mg were
 * dropped (those now live in a separate view, out of scope for this script).
 *
 * How it decides what to fetch:
 *   1. Loops over SELECTED_LEAGUE_IDS (same 57 leagues as the standings
 *      backfill).
 *   2. For each league, reads which seasons already exist in `results`.
...
// Same 122 merged leagues as backfill-standings.js and sync_fixtures_odds.js.
...
async function getExistingSeasons(leagueId) {
  const { data, error } = await supabase.from('results').select('season').eq('league_id', leagueId);
