/**
 * Sync next-7-days fixtures + 1X2 odds into Supabase `soccer_fixtures`.
 ...
 *
 * Run:
 *  node soccer-sync-fixtures-odds.js
 *
 ...
 * moonbet_domestic_leagues.xlsx (used by soccer-backfill-standings.js and
 * soccer-backfill-results.js). Keeps live fixtures/odds coverage in sync
 * with which leagues have full standings/results history.
 */
...
async function enforceWindowAndDeleteMissing(fromISO, toISO, leagueIds, keepIds) {
  ...
  else console.log('🧹 Window enforced: soccer_fixtures table now only has today..+7 days for selected leagues.');
}
