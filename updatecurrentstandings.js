/**
 * Refreshes `current_standings` with each league's live table for its
 * current season. Meant to run on a schedule (cron) so the "current"
 ...
 *     backfill-standings.js, backfill-results.js, and sync_fixtures_odds.js
 *     (union of the original 95-league cron list and the 57-league
 *     shortlist from moonbet_domestic_leagues.xlsx). Keep all four scripts
 *     on this same list going forward.
 ...
 *     backfill-standings.js.
 *
 * Note: current_standings.points2top / mooncentile / moongrade are left
 * alone by this script (as before) - those were populated by a separate
 * process tied to the now-dropped standings.ppg/moongrade columns
 * (current_standings_with_grades, which broke when those columns were
 * removed). That's a separate piece of work if you want live grades back
 * on current_standings - this script only refreshes the raw table.
 *
...
// Same 122 merged leagues as backfill-standings.js, backfill-results.js,
// and sync_fixtures_odds.js.
const LEAGUE_IDS = [
...
async function replaceLeagueRows(leagueId, rows) {
  const { error: delErr } = await supabase.from('current_standings').delete().eq('league_id', leagueId);
  if (delErr) throw new Error(`delete failed: ${delErr.message}`);

  if (rows.length === 0) return; // league had no standings this run - leave it empty, not stale

  const { error: insErr } = await supabase.from('current_standings').insert(rows);
