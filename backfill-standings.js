 ...
 *
 * Fills in missing seasons in `public.standings` from API-Football, for the
 * 122 domestic leagues in SELECTED_LEAGUE_IDS below (the merged union of
 * your moonbet_domestic_leagues.xlsx shortlist and sync_fixtures_odds.js's
 * live cron list), going back to 2009, without re-fetching (or re-paying
 * API calls for) seasons you already have. Existing rows are never touched.
 *
...
// AND the 95 leagues already hardcoded in sync_fixtures_odds.js (the live
// GitHub cron). 122 unique leagues, 30 of which overlapped. Keeping all
// three scripts (this one, backfill-results.js, sync_fixtures_odds.js) on
// the exact same list so standings/results history and live fixtures/odds
...
async function getExistingSeasons(leagueId) {
  const { data, error } = await supabase.from('standings').select('season').eq('league_id', leagueId);
